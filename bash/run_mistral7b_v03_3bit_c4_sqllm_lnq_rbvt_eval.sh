...
trap 'echo "[mistral7b-v03-3bit-c4] FAILED at line ${LINENO}: ${BASH_COMMAND}" >&2' ERR
...
export MODEL_LABEL="${MODEL_LABEL:-mistral7b_v03}"
export MODEL_TYPE="${MODEL_TYPE:-mistral}"
export BIT="${BIT:-3}"
...
export OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/mistral7b_v03_3bit_c4_sqllm_lnq_rbvt}"
export CACHE_ROOT="${CACHE_ROOT:-cache/mistral7b_v03_3bit_c4_sqllm_lnq_rbvt}"
...
export RBVT_INPUT_LUT="${RBVT_INPUT_LUT:-}"
...
for style in ${EVAL_STYLES}; do
  ...
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] [mistral7b-v03-3bit-c4] Running ${PPL_EVAL_STYLE} eval; targets=${PPL_TARGETS}; datasets=${PPL_DATASETS}"
